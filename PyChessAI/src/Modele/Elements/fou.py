...
class Fou(PieceM):
    ...
    def __init__(self, position, couleurBlanc):
        '''
        C'est le constructeur qui va représenter un fou sur l'échiquier
        :param position: c'est la position que le fou occupe sur l'échiquier (c'est une position bidimensionnelle x et y qui peuvent prendre les valeurs entières de 0 à 7)
    ...
    def possibiliteBouger(self, board):
        '''
        Cette méthode a pour utilité de déterminer tous les mouvements que peut faire le fou (sans tenir compte si cela va mettre en danger le roi)
