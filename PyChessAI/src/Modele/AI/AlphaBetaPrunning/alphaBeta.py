                    ...
                    for i in range(len(moves)):
                        for j in range(len(moves[i])):
                            if moves[i][j]:
                                ...
                                Modele.Elements.memoire.Memoire.move_made([i,j], initial, self.board[i][j], tempManger, special)#Indiquer à la mémoire qu'un move fut fait (pour qu'on puisse par la suite undo)
                                score = self.alphaBetaMax(alpha, beta, depthleft-1) #passer à travers l'arbre de décision
                                Modele.Elements.memoire.Memoire.undo(self.board)#undo le mouvement (pour que sa puisse correctement passer à travers l'arbre de décision)
                    ...
                    for i in range(len(moves)):
                        for j in range(len(moves[i])):
                            if moves[i][j]:
                                ...
                                Modele.Elements.memoire.Memoire.move_made([i,j], initial, self.board[i][j], tempManger, special)
